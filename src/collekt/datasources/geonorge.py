...
2: WFS end-to-end
3: Work in progress: download/order API (not completed yet)
"""
...
def fetch_download(record: DatasetRecord, bbox: Bbox, output_dir: Path) -> list[Path]:
    """Order via nedlasting.geonorge.no, (using _place_order, _download_files,
    _unzip_all that we tried in the Fiskeridirektoratet-edition). 
    
    Server-side polygon clip when the dataset capabilities report supportsPolygonSelection.
    Otherwise 'Hele landet' reduced by local crop.
    """
    raise NotImplementedError("download/order API datasets not yet completed")


def crop(paths: list[Path], bbox: Bbox) -> gpd.GeoDataFrame:
    """Clip fetched vector files to buffered window bbox in WGS84.
    ...
    def execute(self,
        ...
        for record in records:
            ...
            if record.kind == "download":
                record.status = "download/order API not yet completed ! Not implemented yet!"
                continue
            try:
