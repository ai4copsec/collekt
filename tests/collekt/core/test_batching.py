...
def test_daily_coverage_attributes_and_storage_chunks_do_not_describe_whole_batch():
    ...
    dataset = xr.Dataset(
        ...
        attrs={
            ...
            "time_coverage_duration": "PT255600S",
    ...
    assert daily.attrs["time_coverage_start"] == "2023-01-02T00:00:00.000000000Z"
    assert daily.attrs["time_coverage_end"] == "2023-01-02T23:00:00.000000000Z"
    assert daily.attrs["time_coverage_duration"] == "PT82800S"
    ...
    assert dataset.attrs["time_coverage_duration"] == "PT255600S"
...
@pytest.mark.parametrize("raw", [{"data_format": "grib"}, {"filename_pattern": "same.nc"}])
def test_unsupported_daily_output_configuration_rejected_in_plan(tmp_path, raw):
